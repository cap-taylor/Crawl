...
### 최근 버전 히스토리
- **1.2.1**: 크롤러 무한 루프 버그 수정 (2025-10-17)
  - TypeError 수정: brand_name이 None일 때 에러 방지
  - 연속 중복 20개 감지 시 자동 스크롤 로직 추가
  - 중복 상품 무한 반복 문제 해결 (수억 개 수집 가능)
  - 스크롤 최적화: 새 상품 로드 시 즉시 스크롤
- **1.2.0**: GUI UI/UX 현대적 개선 (2025-10-17)
  - Material Design 색상 적용 (파란색/주황색 버튼)
  - 상태별 색상 코딩 (대기/수집중/완료/실패/중지)
  - 버튼 크기 40px → 50px 확대
  - 접기 가능한 설정 패널 추가
  - 카드 스타일 디자인 (둥근 모서리)
  - 창 크기 900x800 → 950x850 확대
  - CustomTkinter 이모지 렌더링 문제 해결 (모든 이모지 제거)
  - 텍스트 명확화 및 가독성 개선
- **1.1.4**: 장시간 크롤링 안전성 대폭 개선 (메모리 누수 해결)
  - GUI 로그 줄 수 1000줄 제한 (메모리 누수 방지)
  - 무한 모드에서 100개마다 자동 DB 저장 (데이터 손실 방지)
  - 로그 출력 간결화 (10개마다 상세 로그, 나머지는 기호)
  - 24시간 연속 실행 가능하도록 최적화
  - 테스트 스크립트 추가 (test_long_run_safety.py)
