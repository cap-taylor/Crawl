...
## 🔢 **버전 관리 (VERSION) - 코드 변경 시에만 자동 증가**

### 🚨 중요: 버전 자동 업데이트 규칙

**실제 코드 수정/개선할 때만 자동으로 PATCH 버전 증가!**
- **버전 UP 대상**:
  - ✅ 코드 수정 (`.py`, `.js`, `.ts` 등)
  - ✅ 버그 수정
  - ✅ 기능 개선
  - ✅ 성능 최적화
  - ✅ 설정 파일 변경 (`.env.example`, `config.py` 등)
- **버전 유지 대상**:
  - ❌ 문서만 수정 (`.md` 파일)
  - ❌ 주석만 수정
  - ❌ README, CLAUDE.md, docs/*.md
- **백업은 별개**: 버전 업데이트 후 사용자가 "백업해줘" 요청 시에만 Git 커밋
